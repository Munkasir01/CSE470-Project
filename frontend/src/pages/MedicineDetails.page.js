...
const MedicineDetails = () => {
  ...
  try {
    ...
    if ((medicine.stock ?? 0) <= 0) {
      alert("Cannot add to cart. Medicine is out of stock.");
      return;
    }
    ...
    const cartItem = {
      ...
      userId: id,
      stock: medicine.stock, // Include stock in cart item
    };
    ...
    if (response.ok) {
      if (response.status === 280) {
        ...
        alert(data.alert || "This medicine is already in your cart.");
        return;
      }
      ...
      alert(error.error || "Failed to add medicine to cart.");
    }
  } catch (error) {
